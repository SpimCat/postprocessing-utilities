...
public class DCTS2DPerPixelMeasurement<T extends RealType<T>> implements
                                                                         Command
{
  @Parameter private ImagePlus imp;

  @Parameter private int radius = 10;

  @Parameter private UIService uiService;

  @Parameter(type = ItemIO.OUTPUT) Img<FloatType> localEntropyImg;

  @Override public void run()
  {
    Img<T> real = ImageJFunctions.wrapReal(imp);
    if (real.numDimensions() == 2) {


      localEntropyImg = process2D(real, radius);
    }
    else
    {

      localEntropyImg = process3D(real, radius);
    }
    //uiService.show("stddev", localEntropyImg);
                                                                   ...
                                                                   RandomAccessibleInterval<T> slice)
  {
    ...
    for ( final Neighborhood< T > localNeighborhood : shape.neighborhoods(source ) )
    {
      ...
      double localEntropy = DCTS2DImglib2.entropyOfSlice(cursor, array, lDCTForWidthAndHeight, localWidth, localWidth, pPSFSupportRadius );
